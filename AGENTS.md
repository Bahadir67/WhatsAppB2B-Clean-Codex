...
Keep the multi-agent runtime organized around `src/core/`. The CLI entrypoint `swarm_b2b_system.py` now just boots the stack; the Swarm runtime lives in `swarm_runtime.py`, agent definitions in `swarm_agents.py`, shared state/helpers in `swarm_context.py`, HTML/search/order utilities in `swarm_html.py`, `swarm_search.py`, and `swarm_orders.py`, and Flask routes in `swarm_api.py`. `whatsapp-webhook-sender.js` handles the WhatsApp bridge, `product-list-server-v2.js` serves HTML catalogs, and `config.js` centralizes paths and ports. Database helpers and SQL utilities sit alongside them; avoid scattering new modules outside `src/core` unless they are migrations (`migrations/`) or generated catalog HTML (`product-pages/`). Data seeds and manual SQL live in `sql/`. Session artifacts go under `whatsapp-sessions/`; they should not be committed.
